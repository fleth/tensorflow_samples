...
def make_mini_batch(train_data, size_of_mini_batch, length_of_sequences, size_of_input_data):
    ...
    for _ in range(size_of_mini_batch):
        index   = random.randint(0, len(train_data["inputs"]) - 1)
        inputs_part    = train_data["inputs"][index]
        outputs_part    = math.degrees(train_data["outputs"][index][0])
        inputs  = np.append(inputs, inputs_part)
        outputs = np.append(outputs, outputs_part)
...
#        bias2_var   = tf.Variable(tf.zeros([num_of_output_nodes]), name="bias2")
        ...
        in1 = tf.transpose(input_ph, [1, 0, 2])         # (batch, sequence, size, data) -> (sequence, batch, size, data)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_of_prediction_epochs):
            pred_dict = {
                input_ph:  inputs[epoch].reshape((1, length_of_sequences, 1)),
                istate_ph: states,
            }
