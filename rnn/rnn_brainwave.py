...
def create_data(nb_of_samples, inputs, trainings):
    ...
    t0 = np.transpose(t, [1, 0])
    return X, t0[0]

def make_prediction(nb_of_samples, inputs, trainings):
    xs, ts = create_data(nb_of_samples, inputs, trainings);
    return np.array([[[y] for y in x] for x in xs]), np.array([[x] for x in ts])
...
def calc_accuracy(output_op, inputs_data, training_data, prints=False):
    ...
    inputs, ts = make_prediction(num_of_prediction_epochs, inputs_data, training_data)
    pred_dict = {
        input_ph:  inputs,
        supervisor_ph: ts,
    ...
    def print_result(i, p, q):
        ...
        print(q)
    if prints:
        print(inputs, output[0], ts)
        [print_result(i, p, q) for i, p, q in zip(inputs, output[0], ts)]

    opt = abs(output - ts)[0]
    total = 0
    for x in opt:
        total += sum([1 if y < 0.01 else 0 for y in x])
    print(total)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_of_training_epochs):
            ...
            if (epoch) % 100 == 0:
                ...
                if (epoch) % 500 == 0:
                    calc_accuracy(output_op, brainwaves, mouse["button"])

        calc_accuracy(output_op, brainwaves, mouse["button"], prints=True)
